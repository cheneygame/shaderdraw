...
vec4 getPicPixel()
{
	...
	if(posnum > 0)
	{
		...
		vec4 pixel = texture2D(u_texture2,nCoord); //背景色
		...
		for(int i = 0;i < posnum;i++)  //允许一次传入多点，因此这里用循环
		{
			...
			//vec4 temp = getpixelInRect2(a,npos);
			...
			*/
			//pixel = mix(pixel,temp,1);
			//pixel = max(pixel,temp);
			//pixel = temp;
			vec4 temp = getpixelInRect3(a,npos); //获取笔刷颜色
			//判断当前计算点是否在轨迹上，是：就获取笔刷颜色
			if(temp.w > 0)
			{
				isGetPencilPixel = true;
				//pixel = vec4(mix(pixel.rgb,temp.rgb,temp.a),0.5); //混合背景色和填充色
				//pixel = vec4(mix(pixel.rgb,temp.rgb,temp.a),temp.a); //混合背景色和填充色
				vec4 mixColor = vec4(mix(pixel.rgb,temp.rgb,1-pixel.a),temp.a); //temp.a
				//颜色混合公式系数需要考虑：第一次是透明底图的时候，第二次是非透明时候
				if(pixel.a == 0.0)
				{
				...
				pixel = vec4(mixColor.rgb,temp.a); //mixColor
		...
		vec4 color3 = texture2D(u_texture2,nCoord); //背景色
