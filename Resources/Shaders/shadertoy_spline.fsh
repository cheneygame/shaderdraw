...
void main(void)
{
	...
	SIGNED_DIST = true;//!keyToggle(64+19); 
	gl_FragColor.b += showFlag(vec2(-1.5,.9),uv,(mod(10.*t,2.)<1.))
	...
	if (iMouse.z<=0.) {
		...
		SHOW_MESH   = true;//keyToggle(64+13); // 'M'
		SHOW_LINE   = true;//keyToggle(64+12); // 'L'
		SHOW_GRAD   = false;//!keyToggle(64+7); // 'G'
		SHOW_ISO    = true;//keyToggle(64+9);  // 'I'	
		SHOW_COSTLY = true;//keyToggle(64+3);  // 'C'	
		Pc = mouse; Tc = vec2(1.);
	}
	...
	for (float i=1.; i<float(POINTS); i++) 
	{
		...
		if (dmMi.x==0.) // display mesh
			{ gl_FragColor=mix(vec4(1),vec4(0.,0.,1.,0.),dmMi.y/3.); return; } 
		if (dmMi.x==8.) // display ambiguous parts
			{ gl_FragColor=vec4(.15,0.,0.,0.); return; } 
		dmM = min (dmM, dmMi);
	}
