...
float countV()
{
	...
	for(int i = 0;i < posnum - 1;i++)
	{
		...
		/*
		float len1 = length(npos - a);
		float len2 = length(npos - b);
		if(len1 + len2 < 3.0f)
		{
			v = 1.0;
		}
...
void main(void)
{
	...
	//vec4(v,v,v,1.0) 白色线条
	//vec4(v,v,v,1.0)*vec4(0.5,0.5,0,0.1) 混合色，如黄色
	//color2.x = v;
	//color2.y = v;
	//color2.z = v;
	//v = gl_FragCoord.x/iResolution.x;
	gl_FragColor = vec4(v,v,v,1.0);//vec4(v,v,v,1.0)*vec4(0.5,0.5,0,0.1);//正常情况
	/*
	bool found = countVB();
	if(found)
	{
