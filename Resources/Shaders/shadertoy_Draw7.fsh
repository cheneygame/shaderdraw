...
void main(void)
{
	...
	float uvx = mod(gl_FragCoord.y,u_texture1Size.y)/u_texture1Size.y; //去模可以实现多纹理
	float uvy = mod(gl_FragCoord.x,u_texture1Size.x)/u_texture1Size.x;
	...
	{
		//除了中心外的内容
		gl_FragColor = vec4(v,v,v,1.0)*color2; // line    *vec4(0,0.0,0.5,0.1)
	}else if(v > (1-width)) 
	{
		 //最中心那条线
		gl_FragColor = vec4(v,v,v,1.0)*color2; // line    *vec4(0,0.0,0.5,0.1)
