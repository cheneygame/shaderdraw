...
Sphere GetLightSphere( int lightId ) {
    for( int i=0; i<LIGHT_COUNT; i++ ) {
        if( i == lightId ) {
            return spherelight[i];
        }
    }
    return spherelight[0];
}
...
void initMaterialLibrary()
{
    ...
    INIT_MTL( MTL_LIGHT_1, BSDF_R_LIGHT, 0.0, color*(sin(iGlobalTime*1.3)+1.1) );
...
vec3 calcLightOnSurface( RayHit hit ) {
    ...
    if( surfMtl.bsdf_ == BSDF_R_LIGHT ) {
        ...
        if( hit.materialId == MTL_WALL ) {
            surfColor = texture2D( iChannel0, hit.uv ).xyz;
        } else if( hit.materialId == MTL_SPHERE ) {
            surfColor = texture2D( iChannel1, hit.uv ).xyz;
            specVal = 1.0 - BRIGHTNESS(texture2D( iChannel1, hit.uv ).xyz);
        }
...
void main(void)
{
	...
	for(int si=0; si<PIXEL_SAMPLES; ++si ){
        ...
        vec2 screenCoord = gl_FragCoord.xy + vec2( (1.0/float(PIXEL_SAMPLES))*(float(si)+rnd()), rnd() );
